...
class InMemoryIdentityServiceTests {
    ...
    fun `get anonymous identity by key`() {
        ...
        assertEquals<PartyAndCertificate>(aliceTxIdentity, actual!!)
        ...
        assertEquals<PartyAndCertificate>(bobTxIdentity, actual!!)
    }
    ...
    fun `assert ownership`() {
        withTestSerialization {
            ...
            val anonymousBob = PartyAndCertificate(Party(bob.name, bobTxKey.public), bobTxCert, bobCertPath)
            service.verifyAndRegisterIdentity(anonymousBob)

            // Verify that paths are verified
            service.assertOwnership(alice.party, aliceTxIdentity.party.anonymise())
            ...
            assertFailsWith<IllegalArgumentException> {
                service.assertOwnership(bob.party, aliceTxIdentity.party.anonymise())
            }

            assertFailsWith<IllegalArgumentException> {
                val owningKey = Crypto.decodePublicKey(trustRoot.certificate.subjectPublicKeyInfo.encoded)
                service.assertOwnership(Party(trustRoot.certificate.subject, owningKey), aliceTxIdentity.party.anonymise())
            }
        }
    }
